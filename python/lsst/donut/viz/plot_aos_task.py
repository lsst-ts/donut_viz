...
class PlotDonutFitsTask(pipeBase.PipelineTask):
    ...
    def plotResults(
        ...
        axs[0].text(0.05, 0.05, f"blur: {blur:5.3f}", transform=axs[0].transAxes, fontsize="small", va="bottom")
        axs[1].imshow(models[0], cmap=cmap, vmin=-vmax / 10, vmax=vmax)
        axs[1].text(0.05, 0.05, f"id:  {row['intra_donut_id'][-3:]}", transform=axs[1].transAxes, fontsize="small", va="bottom")
        axs[2].imshow(imgs[0] - models[0], cmap="bwr", vmin=-vmax / 3, vmax=vmax / 3)
        _, _, ttl_res = self.computeResidualStats(imgs[0], models[0])
        axs[2].text(0.05, 0.05, f"res:  {ttl_res:5.3f}", transform=axs[2].transAxes, fontsize="small", va="bottom")
        axs[3].imshow(imgs[1], cmap=cmap, vmin=-vmax / 10, vmax=vmax)
        axs[4].imshow(models[1], cmap=cmap, vmin=-vmax / 10, vmax=vmax)
        axs[4].text(0.05, 0.05, f"id:  {row['extra_donut_id'][-3:]}", transform=axs[4].transAxes, fontsize="small", va="bottom")
        axs[5].imshow(imgs[1] - models[1], cmap="bwr", vmin=-vmax / 3, vmax=vmax / 3)
        _, _, ttl_res = self.computeResidualStats(imgs[1], models[1])
        axs[5].text(0.05, 0.05, f"res:  {ttl_res:5.3f}", transform=axs[5].transAxes, fontsize="small", va="bottom")
